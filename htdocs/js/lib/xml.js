...
function custom_sort(a, b) {
	// compare objects a and b based on sort_args
...
function deep_diff_object(a, b) {
	...
	if (typeof(a.length) == 'undefined') {
		// objs are hashes
		if (num_keys(a) != num_keys(b)) return 1; // different number of hash keys
		for (var akey in a) {
...
function set_path_value(path, obj, value) {
	...
	while (/\/[^\/]+/.test(path)) {
		...
		if (typeof(obj.length) == 'undefined') {
			...
			if (!isa_hash(obj[name])) obj[name] = {}; // force subnode to hash
			obj = obj[name];
		}
		else {
			// obj is array
